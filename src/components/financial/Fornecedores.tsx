...
function StatusBadge({ ok }: { ok?: boolean | string }) {
  const active =
    ok === true || (typeof ok === 'string' && /ativ/i.test(ok));
  return (
...
function PFModal({ hotelId, initial, onClose, onSaved }: PFModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 overflow-y-auto">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-2xl my-4">
        ...
        <form onSubmit={handleSave} className="p-6 space-y-5">
          ...
          {!initial?.id && (
            <div>
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-sm bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded-lg border border-blue-200 dark:border-blue-800 hover:bg-blue-100 transition-colors"
              >
                <Users className="w-4 h-4" />
                Importar do Departamento Pessoal
                {showEmpPicker ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
              </button>
                  ...
                  <div className="p-2 bg-gray-50 dark:bg-gray-900 border-b dark:border-gray-700">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-8 pr-3 py-1.5 text-sm bg-white dark:bg-gray-800 rounded border dark:border-gray-600 focus:outline-none focus:ring-1 focus:ring-blue-500"
                      />
                    </div>
                  </div>
                    ...
                    {loadingEmp ? (
                      ...
                      filteredEmp.map(emp => (
                        <button
                          ...
                          className="w-full text-left px-4 py-2.5 text-sm hover:bg-blue-50 dark:hover:bg-blue-900/20 flex items-center justify-between border-b last:border-0 dark:border-gray-700"
                        >
                          <span className="font-medium text-gray-800 dark:text-white">{emp.name}</span>
                          {emp.cpf && <span className="text-xs text-gray-500">{formatCpf(emp.cpf)}</span>}
                        </button>
                      ))
                    )}
                  </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="label-sm">Município / UF</label>
              <div className="flex gap-2">
                <input className="input-field flex-1" placeholder="Município" value={form.endereco_municipio ?? ''} onChange={e => set('endereco_municipio', e.target.value)} />
                <input className="input-field w-16" placeholder="UF" value={form.endereco_uf ?? ''} onChange={e => set('endereco_uf', e.target.value.toUpperCase())} maxLength={2} />
              </div>
            </div>
            <div className="sm:col-span-2">
              <label className="label-sm">Logradouro</label>
              <div className="flex gap-2">
                <input className="input-field flex-1" placeholder="Rua / Av." value={form.endereco_logradouro ?? ''} onChange={e => set('endereco_logradouro', e.target.value)} />
                <input className="input-field w-24" placeholder="Nº" value={form.endereco_numero ?? ''} onChange={e => set('endereco_numero', e.target.value)} />
              </div>
            </div>
            <div className="sm:col-span-2">
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
              Cancelar
            </button>
            <button type="submit" disabled={saving} className="px-5 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 flex items-center gap-2 transition-colors">
...
function PJModal({ hotelId, initial, onClose, onSaved }: PJModalProps) {
  ...
  const handleLookup = async () => {
    ...
    try {
      ...
      setForm(f => ({
        ...
        cnae_principal_id: r.cnae_principal_id,
        cnae_principal_desc: r.cnae_principal_desc,
        atividade_economica: r.atividade_economica,
        simples_nacional: r.simples_nacional,
        mei: r.mei,
        ibs: r.ibs,
        cbs: r.cbs,
        lista_exclusao: r.lista_exclusao,
        email: r.email,
        telefone: r.telefone,
        ...
        endereco_municipio: r.endereco_municipio,
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!form.razao_social?.trim() && !form.nome_fantasia?.trim()) {
      setError('Razão Social ou Nome Fantasia é obrigatório');
      return;
    }
    setSaving(true);
    setError('');
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-3xl my-4">
        ...
        <form onSubmit={handleSave} className="p-6 space-y-6">
          ...
          <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 border dark:border-gray-700">
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Consulta de CNPJ</p>
            <div className="flex gap-2">
              <input
          ...
          {/* Dados Cadastrais */}
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Dados Cadastrais</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
              ...
              <div>
                <label className="label-sm">Capital Social</label>
          ...
          {/* CNAE / Atividade */}
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Atividade Econômica</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <table className="w-full text-xs">
                  <thead className="bg-gray-50 dark:bg-gray-900">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-gray-500">Principal</th>
                    </tr>
                  </thead>
                  <tbody>
                    {(form.atividade_economica ?? []).map((a, i) => (
                      <tr key={i} className="border-t dark:border-gray-700">
                        <td className="px-3 py-2 font-mono text-gray-700 dark:text-gray-300">{a.id}</td>
          ...
          {/* Regime Tributário */}
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Regime Tributário</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={!!form.simples_nacional} onChange={e => set('simples_nacional', e.target.checked)}
                  className="rounded border-gray-300 text-blue-600" />
                <span className="text-sm text-gray-700 dark:text-gray-300">Simples Nacional</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={!!form.mei} onChange={e => set('mei', e.target.checked)}
                  className="rounded border-gray-300 text-blue-600" />
            ...
            {(form.lista_exclusao ?? []).length > 0 && (
              <div className="mt-3">
                <p className="text-xs text-gray-500 mb-1">Lista de Exclusão do Simples Nacional</p>
                <div className="space-y-1">
                  {(form.lista_exclusao ?? []).map((ex, i) => (
                    <div key={i} className="text-xs px-3 py-1.5 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 rounded">
          ...
          {/* Contato */}
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Contato</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="label-sm">E-mail</label>
                <input className="input-field" type="email" value={form.email ?? ''} onChange={e => set('email', e.target.value)} />
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={saving} className="px-5 py-2 text-sm bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50 flex items-center gap-2 transition-colors">
              {saving && <Loader2 className="w-4 h-4 animate-spin" />}
              Salvar
            </button>
          </div>
...
export default function Fornecedores() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await supplierService.list(selectedHotel.id);
      setSuppliers(data);
    } catch (err: any) {
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div className="flex gap-2">
          ...
          >
            ...
            Pessoa Jurídica
            <span className={`px-1.5 py-0.5 rounded-full text-xs font-bold ${typeTab === 'juridica' ? 'bg-white/20 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}>
              {pjCount}
            </span>
          </button>
          <button
          ...
          >
            ...
            Pessoa Física
            <span className={`px-1.5 py-0.5 rounded-full text-xs font-bold ${typeTab === 'fisica' ? 'bg-white/20 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}>
              {pfCount}
            </span>
          </button>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-3 py-2 text-sm bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <button onClick={load} disabled={loading} className="p-2 text-gray-500 hover:text-gray-700 dark:hover:text-gray-200">
            <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
          </button>
          <button
            onClick={() => typeTab === 'juridica' ? setModalPJ({ open: true }) : setModalPF({ open: true })}
            className={`flex items-center gap-2 px-4 py-2 text-sm text-white rounded-lg transition-colors ${typeTab === 'juridica' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-blue-600 hover:bg-blue-700'}`}
          >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-3">
          {filtered.map(s => (
            <div key={s.id} className="bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
              {s.type === 'juridica' ? (
                <PJCard
                  supplier={s}
...
function PJCard({ supplier: s, onEdit, onDelete, deleting }: CardProps) {
  ...
  return (
    <div className="p-4">
        ...
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-1">
              ...
              {s.simples_nacional && (
                <span className="px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs rounded-full">Simples</span>
              )}
              {s.mei && (
                <span className="px-1.5 py-0.5 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 text-xs rounded-full">MEI</span>
              )}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <button onClick={() => setExpanded(p => !p)} className="p-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 rounded">
            {expanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
          </button>
          <button onClick={onEdit} className="p-1.5 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 rounded">
            <Edit2 className="w-4 h-4" />
          </button>
          <button onClick={onDelete} disabled={deleting} className="p-1.5 text-gray-400 hover:text-red-600 dark:hover:text-red-400 rounded disabled:opacity-50">
      ...
      {expanded && (
        <div className="mt-4 pt-4 border-t dark:border-gray-700 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
          ...
          {(s.lista_exclusao ?? []).length > 0 && (
            <div className="sm:col-span-2 lg:col-span-3">
              ...
              {(s.lista_exclusao ?? []).map((ex, i) => (
                <span key={i} className="inline-block text-xs px-2 py-0.5 bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-300 rounded mr-1 mb-1">
                  {ex.date} {ex.text}
                </span>
              ))}
            </div>
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-0.5">
            ...
            {s.employee_id && (
              <span className="text-xs px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-300 rounded-full">DP</span>
            )}
