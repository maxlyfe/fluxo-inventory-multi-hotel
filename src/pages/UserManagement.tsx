    ...
    <>
      ...
      {open && createPortal(
        ...
        >
          ...
          {item('Acesso a hotéis', <Building2 className="h-4 w-4 text-teal-500" />, onManageHotels)}
...
const UserManagement = () => {
  ...
  // Acesso a hotéis por usuário (user_hotel_access)
  const [accessUser, setAccessUser]       = useState<User | null>(null);
  const [accessHotels, setAccessHotels]   = useState<{ id: string; name: string; code: string }[]>([]);
  ...
  // Abre o modal de acesso a hotéis e carrega hotéis (do grupo, via RLS) + concessões
  const openHotelAccess = async (u: User) => {
    setAccessUser(u);
  ...
  const toggleHotelAccess = async (hotelId: string) => {
    if (!accessUser) return;
    const granted = accessGranted.has(hotelId);
    setAccessBusy(hotelId);
    try {
      if (granted) {
        const { error } = await supabase.from('user_hotel_access')
          .delete().eq('user_id', accessUser.id).eq('hotel_id', hotelId);
        if (error) throw error;
        setAccessGranted(prev => { const n = new Set(prev); n.delete(hotelId); return n; });
      } else {
        const { error } = await supabase.from('user_hotel_access')
          .insert({ user_id: accessUser.id, hotel_id: hotelId });
        if (error) throw error;
        setAccessGranted(prev => new Set(prev).add(hotelId));
      }
    } catch (e: any) {
      showToast('error', 'Erro ao atualizar acesso: ' + e.message);
    } finally {
      setAccessBusy(null);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-3 sm:px-6 py-6 sm:py-8 space-y-5">
                  ...
                  return (
                    <div key={user.id}
                      ...
                      <UserActionsMenu
                        ...
                        onManageHotels={() => openHotelAccess(user)}
      ...
      {/* ══ Modal — Acesso a hotéis do usuário ══ */}
      {accessUser && (
        <Modal title="Acesso a hotéis" subtitle={accessUser.email} onClose={() => setAccessUser(null)}>
          <div className="space-y-3">
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Marque os hotéis do grupo que este usuário pode acessar. Admins do grupo já têm
              acesso a todos os hotéis ativos automaticamente.
            </p>
            {accessLoading ? (
              <div className="flex items-center justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-blue-500" /></div>
            ) : accessHotels.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-6">Nenhum hotel disponível no grupo.</p>
            ) : (
              <div className="space-y-1.5 max-h-[55vh] overflow-y-auto">
                {accessHotels.map(h => {
                  const granted = accessGranted.has(h.id);
                  return (
                    <button key={h.id} onClick={() => toggleHotelAccess(h.id)} disabled={accessBusy === h.id}
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors ${granted ? 'bg-teal-50 dark:bg-teal-900/20' : 'hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}>
                      <span className={`w-5 h-5 rounded-md border flex items-center justify-center shrink-0 ${granted ? 'bg-teal-600 border-teal-600' : 'border-slate-300 dark:border-slate-600'}`}>
                        {accessBusy === h.id ? <Loader2 className="w-3 h-3 animate-spin text-slate-400" /> : granted ? <CheckCircle className="w-3.5 h-3.5 text-white" /> : null}
                      </span>
