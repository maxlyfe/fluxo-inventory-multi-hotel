    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          purchase_items ( id, product_id, quantity, unit_price, total_price, ncm, cean, ibs, cbs, products:products(id, name) ),
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-slate-50 dark:bg-slate-800/60 text-left">
                            ...
                            <th className="px-4 py-2.5 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">cEAN</th>
                            <th className="px-4 py-2.5 text-xs font-semibold text-teal-600 dark:text-teal-400 uppercase tracking-wide text-right">IBS</th>
                            <th className="px-4 py-2.5 text-xs font-semibold text-purple-600 dark:text-purple-400 uppercase tracking-wide text-right">CBS</th>
                        ...
                        <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                          {p.purchase_items.map(item => (
                            <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                              ...
                              <td className="px-4 py-2.5 text-right font-mono text-xs text-teal-700 dark:text-teal-300">
                                {item.ibs != null && Number(item.ibs) > 0
                                  ? `R$ ${Number(item.ibs).toFixed(2)}`
                                  : <span className="text-slate-300 dark:text-slate-600">—</span>}
                              </td>
                              <td className="px-4 py-2.5 text-right font-mono text-xs text-purple-700 dark:text-purple-300">
                                {item.cbs != null && Number(item.cbs) > 0
                                  ? `R$ ${Number(item.cbs).toFixed(2)}`
                                  : <span className="text-slate-300 dark:text-slate-600">—</span>}
                              </td>
