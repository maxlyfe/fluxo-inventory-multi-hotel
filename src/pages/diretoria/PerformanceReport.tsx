// src/pages/diretoria/PerformanceReport.tsx
// Análise comparativa de KPIs por período — anteriormente chamado de "Pick-up".
// O verdadeiro relatório de Pick-up (velocidade de reservas + snapshots OTB) está em PickupReport.tsx.
      ...
      <div style={{ borderBottom: `1px solid ${isDark ? 'rgba(148,163,184,0.08)' : 'rgba(203,213,225,0.5)'}`, padding: '1.25rem 2rem', background: isDark ? 'rgba(2,6,23,0.85)' : 'rgba(255,255,255,0.85)', backdropFilter: 'blur(20px)', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              <h1 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 900, color: isDark ? '#f8fafc' : '#1e293b', letterSpacing: -0.8 }}>Análise de Performance</h1>
              <p style={{ margin: 0, fontSize: 13, color: isDark ? '#94a3b8' : '#64748b', fontWeight: 500 }}>Comparativo de períodos e KPIs hoteleiros</p>
            </div>
          </div>
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '2.5rem 2rem' }}>
        ...
        <div style={{ marginBottom: '3rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
            ...
            {periods.length < 5 && <button onClick={addPeriod} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0.6rem 1.2rem', background: 'rgba(139,92,246,0.1)', border: '1px solid rgba(139,92,246,0.3)', borderRadius: 12, color: '#8b5cf6', fontSize: 14, fontWeight: 700 }}><Plus size={16} /> Adicionar</button>}
          </div>
          ...
          <div style={{ marginTop: '2.5rem', display: 'flex', justifyContent: 'center' }}>
            <button className="pickup-search-btn" onClick={handleSearch} disabled={loading} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '1rem 3.5rem', borderRadius: 16, border: 'none', background: 'linear-gradient(135deg, #8b5cf6, #7c3aed)', color: '#fff', fontWeight: 800, fontSize: '1.1rem', cursor: 'pointer', boxShadow: '0 10px 25px -5px rgba(139,92,246,0.4)' }}>
          ...
          <div style={{ animation: 'fadeIn 0.5s ease' }}>
            <div style={{ display: 'flex', gap: 6, background: isDark ? 'rgba(15,23,42,0.8)' : 'rgba(255,255,255,0.8)', border: `1px solid ${isDark ? 'rgba(148,163,184,0.1)' : 'rgba(203,213,225,0.5)'}`, borderRadius: 16, padding: 6, width: 'fit-content', marginBottom: '2.5rem' }}>
              {(['individual', 'comparativo'] as const).map(v => (
                <button key={v} className="pickup-tab" onClick={() => setView(v)} style={{ padding: '0.75rem 1.75rem', borderRadius: 12, border: 'none', background: view === v ? '#8b5cf6' : 'transparent', color: view === v ? '#fff' : isDark ? '#94a3b8' : '#64748b', fontWeight: 800, fontSize: 14 }}>
                        ...
                        <tr key={row.key} style={{ background: isDark ? 'rgba(30,41,59,0.1)' : 'rgba(241,245,249,0.5)', borderRadius: 24 }}>
                          <td style={{ padding: '2rem 1.5rem', borderTopLeftRadius: 24, borderBottomLeftRadius: 24, minWidth: 160 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12, color: isDark ? '#94a3b8' : '#475569', fontWeight: 700 }}>
                              <div style={{ color: '#8b5cf6' }}>{row.icon}</div>
                              {row.label}
                            </div>
                          </td>
