  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between mb-3">
        ...
        {href && (
          <Link to={href} className="text-xs text-indigo-500 hover:text-indigo-600 dark:text-indigo-400 dark:hover:text-indigo-300 flex items-center gap-0.5 transition-colors">
...
function MyScheduleWidget({ entries, loading }: { entries: ScheduleEntry[]; loading: boolean }) {
  if (loading) return (
    <WidgetCard title="Minha Escala" icon={Clock} color="#8b5cf6">
      <LoadingPlaceholder />
    </WidgetCard>
  );
  ...
  const typeColors: Record<string, string> = {
    shift:      'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300',
    folga:      'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300',
    ...
    inss:       'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300',
      ...
      <div className="space-y-1">
        {days.map(day => {
          ...
          return (
            <div
              key={dateStr}
              className={`flex items-center justify-between py-1.5 px-2.5 rounded-xl text-sm transition-colors ${
                isCurrent
            ...
            >
              <div className="flex items-center gap-2 min-w-0">
                <span className={`text-xs font-semibold w-10 shrink-0 ${isCurrent ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'}`}>
              ...
              {entry ? (
                entry.entry_type === 'shift' && entry.shift_start && entry.shift_end ? (
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${typeColors.shift}`}>
                    {entry.shift_start.slice(0, 5)} — {entry.shift_end.slice(0, 5)}
                  </span>
                ) : (
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${typeColors[entry.entry_type] || 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300'}`}>
                    {entry.custom_label || typeLabels[entry.entry_type] || entry.entry_type}
                  </span>
                )
              ) : (
...
function BirthdaysWidget({ employees, loading }: { employees: Employee[]; loading: boolean }) {
  if (loading) return (
    <WidgetCard title="Aniversariantes" icon={Gift} color="#ec4899">
      <LoadingPlaceholder />
    </WidgetCard>
  );
        ...
        <div className="space-y-2 max-h-52 overflow-y-auto pr-1">
          {monthBirthdays.map(emp => {
            ...
            return (
              ...
              >
                {isTodayBday && <PartyPopper className="w-4 h-4 text-pink-500 shrink-0" />}
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-slate-800 dark:text-white truncate">{emp.name}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {emp.sector} · {format(bday, "dd 'de' MMMM", { locale: ptBR })}
                  </p>
                </div>
              </div>
            );
...
function EventsWidget({ events, loading }: { events: EventItem[]; loading: boolean }) {
  if (loading) return (
    <WidgetCard title="Próximos Eventos" icon={Calendar} color="#6366f1">
      <LoadingPlaceholder />
    </WidgetCard>
  );
  ...
  return (
    <WidgetCard title="Próximos Eventos" icon={Calendar} href="/portal/events" color="#6366f1">
      {events.length === 0 ? (
        <p className="text-sm text-slate-400 dark:text-slate-500 text-center py-4">Nenhum evento próximo</p>
      ) : (
        <div className="space-y-2">
          {events.slice(0, 5).map(ev => (
            <div key={ev.id} className="flex items-start gap-2.5 py-1 px-1 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
              <div
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-slate-800 dark:text-white truncate">
                  {ev.title}
                  {ev.is_mandatory && <span className="ml-1.5 text-[10px] bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400 px-1.5 py-0.5 rounded-full font-medium">Obrigatório</span>}
                </p>
                <p className="text-xs text-slate-500 dark:text-slate-400">
...
function MotivationalWidget({ messages, loading }: { messages: MotivationalMessage[]; loading: boolean }) {
  if (loading) return (
    <WidgetCard title="Inspiração" icon={Sparkles} color="#f59e0b">
      <LoadingPlaceholder />
    </WidgetCard>
  );
    ...
    <WidgetCard title="Inspiração do Dia" icon={Sparkles} color="#f59e0b">
      {message ? (
        <div className="text-center py-3 px-2">
          ...
          {message.author && (
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-2">— {message.author}</p>
          )}
        </div>
      ) : (
        <p className="text-sm text-slate-400 dark:text-slate-500 text-center py-4">Nenhuma mensagem configurada</p>
      )}
    </WidgetCard>
  );
}
...
// ---------------------------------------------------------------------------
// Widget: Últimas Entregas de Uniforme
// ---------------------------------------------------------------------------
function DocumentsWidget({ deliveries, loading }: { deliveries: UniformDelivery[]; loading: boolean }) {
  if (loading) return (
    <WidgetCard title="Meus Documentos" icon={FileText} color="#10b981">
      <LoadingPlaceholder />
    </WidgetCard>
  );
  ...
  return (
    <WidgetCard title="Últimas Entregas" icon={Shirt} href="/portal/my-documents" color="#10b981">
      {deliveries.length === 0 ? (
        <p className="text-sm text-slate-400 dark:text-slate-500 text-center py-4">Nenhuma entrega registrada</p>
      ) : (
        <div className="space-y-2">
          {deliveries.slice(0, 3).map(d => (
            <div key={d.id} className="flex items-center justify-between py-1.5 px-2 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
              ...
              <span className="ml-2 text-xs bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300 px-2 py-0.5 rounded-full font-medium shrink-0">
