...
import { searchReservation, saveGuestsToStorage, WebCheckinGuest } from './webCheckinService';
...
export default function WCIReservationSearch() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!query.trim() || !hotelId) return;
    setLoading(true);
    setError('');
    try {
      const result = await searchReservation(hotelId, query);
      if (!result) {
        setError(t('notFound'));
        return;
      }
      ...
      <div style={{ width: '100%', maxWidth: 600 }}>
        ...
        <div style={glassCard}>
          <form onSubmit={handleSearch} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            >
              {loading ? <><Loader2 size={20} style={{ animation: 'spin 1s linear infinite' }} /> {t('searching')}</> : <><Search size={20} /> {t('search')}</>}
            </button>
          </form>
        </div>
