...
type ViewMode = 'map' | 'otb' | 'occupancy';
...
const Planning: React.FC = () => {
  ...
  return (
    <div className="container mx-auto p-4 md:p-6">
      ...
      {/* Filtros de período — apenas para as visões tabulares */}
      {viewMode !== 'map' && (
      ...
      {viewMode !== 'map' && error && <p className="text-red-500 mb-4 text-sm">{error}</p>}

      {viewMode === 'map' ? (
        /* ═══ MAPA DE HOSPEDAGEM (estilo Desbravador) ═══ */
        <PlanningMap hotelId={selectedHotel?.id || ''} erbonConfigured={erbonConfigured} />
      ) : loading ? (
