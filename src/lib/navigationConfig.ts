...
//
// Estrutura: NavCategory (separador fixo) > NavGroup (accordion) > NavItem (link)
...
// ---------------------------------------------------------------------------
// Configuração central — hierarquia: Categoria > Módulo > Acesso
// ---------------------------------------------------------------------------
    ...
    groups: [
      {
        ...
        items: [
          ...
          { module: 'diretoria', label: 'Vendas & Ocupação', href: '/diretoria/vendas', icon: DollarSign, iconName: 'DollarSign', color: '#10b981' },
    ...
    groups: [
      {
        ...
        icon: SettingsIcon,
        adminOnly: true,
        activePrefixes: ['/users', '/admin/roles', '/admin/sectors', '/admin/groups', '/admin/erbon', '/admin/omnibees', '/admin/whatsapp', '/admin/fnrh', '/admin/nfe'],
        items: [
          ...
          { module: 'omnibees_pms',       label: 'Omnibees',          href: '/admin/omnibees', icon: Globe,         iconName: 'Globe',         color: '#2563eb' },
...
// Derivação flat — backward compat para Navbar, Home.tsx e demais consumidores
export const NAV_GROUPS: NavGroup[] = NAV_CATEGORIES.flatMap(c => c.groups);
