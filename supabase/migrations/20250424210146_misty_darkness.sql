/*
  # Fix Zero Quantity Handling in Sector Stock Balance

  1. Changes
    - Update handle_sector_stock_balance function to properly handle zero quantities
    - Ensure items with zero quantity are removed from sector_stock
    - Add proper validation and error handling
    - Fix issue with null product names
...
BEGIN
  ...
  SELECT COALESCE(name, 'Unknown Product'), COALESCE(category, 'Unknown Category') 
